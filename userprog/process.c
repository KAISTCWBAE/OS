...
/*================================================== IMPLEMENTATION START ==================================================*/
	if (parent->fd_idx >= FDCOUNT_LIMIT) goto error;
	...
	for (int i = 2; i < FDCOUNT_LIMIT; i++) {
		...
		if (f != NULL) current->fd_table[i] = file_duplicate(f);
	}
...
/*================================================== IMPLEMENTATION START ==================================================*/
static void
argument_stack (struct intr_frame *if_, int argv_cnt, char **argv_list)
{
	char *arg_addr[128];
	int argc_len;
	...
	while (if_->rsp % 8 != 0) memset (--(if_->rsp), 0, sizeof (uint8_t));

	for (int i = argv_cnt; i >= 0; i--) {
		if_->rsp -= 8;
		if (i == argv_cnt) memset (if_->rsp, 0, sizeof (char **));
		else memcpy (if_->rsp, &arg_addr[i] , sizeof (char **));
	}
	if_->rsp -= 8;
...
/*================================================== IMPLEMENTATION  END  ==================================================*/
	for (int i = 0; i < FDCOUNT_LIMIT; i++) close(i);
